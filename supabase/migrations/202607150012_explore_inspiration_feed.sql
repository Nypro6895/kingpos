create index if not exists salon_profile_media_assets_public_content_idx
...
as $$
  ...
  look_items as (
    select
      ...
      assets.object_path as media_path,
    ...
    join public.salon_profile_media_assets assets
    ...
    join storage.objects storage_objects
      on storage_objects.bucket_id = assets.bucket
      and storage_objects.name = assets.object_path
    ...
    where looks.status = 'published'
      ...
      and (
        ...
        or lower(assets.object_path) like '%.jpg'
        or lower(assets.object_path) like '%.jpeg'
        or lower(assets.object_path) like '%.png'
        or lower(assets.object_path) like '%.webp'
      )
  ),
  update_items as (
    select
...
comment on function public.get_public_explore_inspiration(
  ...
  uuid,
  integer
) is 'Public, cursor-paginated Explore inspiration feed from published salon profile looks and updates with active validated media.';
