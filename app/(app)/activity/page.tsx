...
function ActivityCard({ activity }: { activity: CustomerActivity }) {
  ...
  return (
    <article className="rounded-2xl border border-border-subtle bg-surface p-4 shadow-sm transition hover:border-brand-orange/40 hover:shadow-[0_16px_36px_rgba(23,19,22,0.06)]">
      ...
      {activity.type === "purchase" && verifiedVisit ? (
        <VisitExperiencePrompt
          compact
          countsTowardReputation={verifiedVisit.countsTowardReputation}
          initialBody={verifiedVisit.experienceBody}
          initialState={verifiedVisit.experienceState}
          salonName={activity.salon.name}
          ticketId={activity.ticketId}
          windowDays={verifiedVisit.windowDays}
        />
      ) : null}
    </article>
  );
...
export default async function ActivityPage() {
  const result = await getCustomerActivity({ limit: 100 });
      ...
      <div className="mx-auto grid w-full max-w-5xl gap-5">
        {!hasActivity ? (
          ...
          <div className="grid gap-7">
            ...
            <ActivityHistoryPanel activities={activity.history} />
          </div>
        )}
      </div>
    </main>
