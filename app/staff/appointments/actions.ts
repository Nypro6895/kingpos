...
export async function updateStaffOnlineBookingAction(formData: FormData) {
  const salonId = readString(formData, "salon_id");
  const nextEnabled = readString(formData, "online_booking_enabled") === "true";
  ...
  const { data, error } = await supabase.rpc("set_own_staff_online_booking", {
    p_online_booking_enabled: nextEnabled,
    p_salon_id: salonId,
  });

  if (error) {
    console.error("Update staff online booking failed", { message: error.message });
    return;
  }
  ...
  if (!payload?.ok) {
    logRejected("Update staff online booking", payload);
    return;
  }
